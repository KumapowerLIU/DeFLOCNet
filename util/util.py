...
def create_gMask(gMask_opts):
    ...
    image_size = gMask_opts['image_size']
    maxPartition=gMask_opts['maxPartition']
    if pattern is None:
        raise ValueError
    wastedIter = 0
    while True:
        x = random.randint(1, MAX_SIZE-image_size)
        y = random.randint(1, MAX_SIZE-image_size)
        mask = pattern[y:y+image_size, x:x+image_size] # need check
        area = mask.sum()*100./(image_size*image_size)
...
def color_mask(inMask, conv_layers):
    ...
    with torch.no_grad():
        ...
        output_mask  = lnet(inMask)
    update_holes = output_mask == 0
    #
    # new_mask = torch.ones_like(output_mask)
    # new_mask = new_mask.masked_fill_(no_update_holes, 0.0).bool()
    return update_holes
...
def mkdir(path):
    if not os.path.exists(path):
        os.makedirs(path)
def data_process(sketch_path, real_img_path,mask_path):
    file_name = sketch_path  # sketch image
    file_name_real = real_img_path  # real image
    mask_file = mask_path  # mask image
    mask = cv2.imread(mask_file)[:, :, 0]
    mask = cv2.resize(mask, (256, 256))
    img_sketch = cv2.imread(file_name)
    img_real = cv2.imread(file_name_real)
    img_sketch = img_sketch[:, :, 0]
    img_sketch_fan = img_sketch.copy()
    for i in range(img_sketch.shape[0]):
        for k in range(img_sketch.shape[1]):
            if img_sketch[i, k] == 255:
                img_sketch[i, k] = 0
            else:
                img_sketch[i, k] = 1

    # for i in range(img_sketch_fan.shape[0]):
    #     for k in range(img_sketch_fan.shape[1]):
    #         if img_sketch_fan[i, k] == 255:
    #             img_sketch_fan[i, k] = 0
    #         else:
    #             img_sketch_fan[i, k] = 255
    ...
    out = mask * img_sketch
    label_f = label(out, connectivity=1)
    num = label_f.max()
    color_s = np.zeros((out.shape[0], out.shape[1], num))
    for i in range(out.shape[0]):
        for k in range(out.shape[1]):
            for s in range(1, num + 1):
                if label_f[i, k] == s:
                    color_s[i, k, s - 1] = 1
    final_color = np.zeros((img_sketch.shape[0], img_sketch.shape[1], 3))
    for i in range(num):
        index = np.argwhere(color_s[:, :, i] > 0)
        if (np.sum(color_s[:, :, i]) > 300):
            color_kernel = np.expand_dims(color_s[:, :, i], 2).repeat(3, axis=2)
            img_ke = color_kernel * img_real
            img_point = np.sum(img_ke, axis=0)
            img_point = np.sum(img_point, axis=0)
            img_point = img_point / len(index)
            img_point = np.rint(img_point)
            num_all_point = len(index)

            point_sample = random.sample(range(0,num_all_point-300),1)[0]
            #oint_sample = len(index)

            for k in range(point_sample,point_sample+300):
                position = index[k]
                x = position[0]
                y = position[1]
                final_color[x, y, :] = img_point


    input_sketch = mask_original * img_sketch_fan
    input_sketch = np.expand_dims(input_sketch, 2).repeat(3, axis=2)
    sketch = Image.fromarray(cv2.cvtColor(input_sketch.astype('uint8'),cv2.COLOR_BGR2RGB))
    color = Image.fromarray(cv2.cvtColor(final_color.astype('uint8'),cv2.COLOR_BGR2RGB))
